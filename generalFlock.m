...
% Control Flock
P.Control = 0;              % toggle control on flock
...
save = 1;           % logical operator, 1 for saving into folder and no pause
...
% Saving
FileLocation='F:\Summer 2016\MCTP\Flocking-Control\Save';%where to create new folder
NewFolder='ImpactDogCircle';%name of new folder to be created
...
for n=0:P.dt:t
    ...
    if (P.choiceBC==2)
        [X_new,V_new] = RK4birds_noBC(X,V,P);
    ...
    elseif (P.choiceBC==10)
        [X_new,V_new] = RK4birds_noBC(X,V,P);
        [X,V] = squarePortal(X,V,X_new,V_new,P);
    else
        [X,V] = RK4birds_withBC(X,V,P);
    end
...
%     varV(round(n/P.dt+1)) = varbirds(V);
    ...
    if shouldPlot
        ...
        if ~isempty(Bound)
            hold on;
            plot(Bound(1,:),Bound(2,:),'k')         %plot the boundary
            hold off;
        end
